...
export default function RadiusSliderHero({ value, onChange }: { value: number; onChange: (v: number) => void }) {
  ...
  return (
    <div className="flex flex-col gap-2 w-full h-full max-w-md items-start justify-center px-4">
      <label htmlFor="radius-input" className="block text-sm font-medium text-white mb-1">
        Select travel radius (km)
      </label>

      <div className="flex items-center gap-3 w-full">
        <button title="Decrease radius"
          onClick={() => setRadius((prev) => Math.max(prev - 1, 1))}
        ...
        <button title="Increase radius"
          onClick={() => setRadius((prev) => Math.min(prev + 1, 30))}
          className="p-2 rounded-full bg-gray-700 text-white hover:bg-gray-600"
        >
      ...
      <p className="text-gray-300 font-medium text-sm">Current radius: {radius} km</p>
    </div>
  );
}
